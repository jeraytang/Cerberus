...
using IdentityServer4.Storage.MySql.TokenCleanup;
...
namespace IdentityServer4.MySql
{
	...
	public class TokenCleanupHost : IHostedService
	{
		...
		public TokenCleanupHost(IServiceProvider serviceProvider,
			ILogger<TokenCleanupHost> logger)
		{
			_serviceProvider = serviceProvider ?? throw new ArgumentNullException(nameof(serviceProvider));
			using var scope = _serviceProvider.CreateScope();
			_options = scope.ServiceProvider.GetRequiredService<IdentityServerOptions>();
			_logger = logger;
		}
