...
namespace IdentityServer4.SecurityTokenService.Endpoints
{
    ...
    internal class TokenEndpoint : IEndpointHandler
    {
        ...
        public TokenEndpoint(
            ...
            ILogger<TokenEndpoint> logger, ISystemClock clock)
        {
        ...
        private async Task<IEndpointResult> ProcessTokenRequestAsync(HttpContext context)
        {
            ...
            var sub = form.Get("sub");
            if (string.IsNullOrWhiteSpace(sub))
            {
                return Error("sub is necessary");
            }

            var authenticationMethod = form.Get("amr");
            ...
            {
                IdentityProvider = "Cerberus",
                AuthenticationTime = _clock.UtcNow.DateTime,
                AuthenticationMethods = new List<string> {authenticationMethod}
            };
            requestResult.ValidatedRequest.Subject = user.CreatePrincipal();
